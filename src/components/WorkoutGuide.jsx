...
const WorkoutGuide = () => {
  ...
  return (
    <div className="dark:bg-Navy">
        ...
        <div className=" bg-[url('/src/assets/Fitness-image-3.png')] grid grid-cols-1 md:grid-cols-2 p-8 gap-4 w-full">
          {exercises.length > 0 ? (
            exercises.map((exercise) => (
              <div key={exercise.id} className="exercise-card border border-gray-200 bg-Navy mix-blend-screen rounded-lg p-4 hover:shadow-lg transition">
                <h3 className="text-xl text-Yellow font-bold">{exercise.name}</h3>
                <p className="text-white font-light">{exercise.description || "No description available."}</p>
      ...
      <p className="text-sm italic  text-center font-thin mt-10 mb-10 text-Navy dark:text-Yellow">“Unleash your potential—find a new challenge with every search!”</p>
    </div>
  );
